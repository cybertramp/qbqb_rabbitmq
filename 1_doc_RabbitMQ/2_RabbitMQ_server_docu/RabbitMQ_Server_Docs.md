...
- **Key Manager, Trust Manager 및 stores**
...
- **"node"정책 및 마스터 마이그레이션(“nodes” Policy and Migrating Masters)**
  ...
  “nodes” 정책을 설정하거나 수정하면 기존 마스터가 새 정책에 나열되지 않은 경우 없어질 수 있다. 메시지 손실을 막기 위해 RabbitMQ는 적어도 하나의 다른 미러가 동기화될 때까지(오랜 시간이더라도) 기존 마스터를 계속 유지한다. 그러나 일단 동기화가 완료되면 마치 노드가 실패한 것처럼 처리가 진행된다. 즉, consumer는 마스터로부터 연결이 끊어지고 다시 연결해야 한다.



- **독점 큐 미러링(Mirroring of Exclusive Queues)**

  독점(Exclusive) 큐는 큐가 선언된 연결이 닫히면 삭제된다. 이런 이유 때문에 독점 큐를 미러링하는 것은 유용하지 않다. 왜냐하면 독점 큐를 호스팅하는 노드가 중단되면 연결이 닫히고 어쨌든 큐를 삭제해야 하기 때문이다. 이러한 이유 때문에 독점 큐는 결코 미러링되지 않는다. 



- **클러스터에서 비 미러링된 큐 동작(Non-mirrored Queue Behavior in a Cluster)**
...
- **미러링된 큐 구현 및 의미(Mirrored Queue Implementation and Semantics)**

  미러링된 각 큐에는 각각 다른 노드에 하나의 마스터 및 여러 개의 미러가 있다. 미러는 마스터와 정확히 같은 순서로 마스터에서 발생하는 동작을 적용하여 동일한 상태를 유지한다. 게시 이외의 모든 작업은 마스터에게만 가고 마스터는 동작(actions)의 결과를 미러에 브로드캐스트한다. 따라서 미러링된 큐로부터 소비하는 클라이언트는 사실 마스터로부터 소비된다.

  미러가 실패(fail)하면 일부 bookkeeping이외에는 할 일이 거의 없다. 마스터는 마스터로 남고 클라이언트는 어떠한 조치나 실패에 대해 알릴 필요가 없다. 미러 실패는 즉시 감지되지 않을 수 있고, 연결당 흐름 제어 메커니즘의 중단으로 인해 메시지 게시가 지연될 수 있다.



- **게시자 확인 및 전달(Publisher Confirms and Transactions)**

  미러링된 큐는 게시자 확인 및 트랜잭션을 모두 지원한다. 선택된 의미(semantics)는 확인 및 트랜잭션의 모두의 경우 동작이 큐의 모든 미러에 걸쳐있다. 그래서 트랜잭션의 경우 트랜잭션이 큐의 모든 미러에 적용될 때 <u>tx.commit-ok</u>는 클라이언트에만 돌려보내진다. 마찬가지로 게시자 확인의 경우 메시지는 모든 미러에 의해 받아들여졌을 때 게시자(publisher)에게만 확인된다. 



- **흐름 제어(Flow Control)**

  RabbitMQ는 신용 기반 알고리즘을 사용하여 메시지 게시 속도를 제한한다. producer는 큐의 모든 미러로부터 신용(credit)을 받을 때 게시가 가능하게 된다. 이 맥락에서의 신용(credit)은 게시 허가를 의미한다. 신용을 발행하지 못하는 미러는 게시자(publisher)를 지연시킬 수 있다. 게시자는 모든 미러가 신용을 발행하거나 남아 있는 노드가 미러를 클러스터로부터 연결을 해제한다고 고려될 때까지 차단된다. Erlang은 주기적으로 모든 노드에 틱(tick)을 전송하여 이러한 연결 해제를 감지한다. 틱 간격은 net_ticktime 설정으로 제어할 수 있다.



- **마스터 실패 및 소비자 취소(Master Failures and Consumer Cancellation)**

  미러링된 큐에서 소비하는 클라이언트는 그들(클라이언트)이 소비한 큐가 페일 오버되었음을 알고자 할 수 있다. 미러링된 큐가 페일 오버되면 어떤 메시지가 어떤 consumer에게 전송되었는지에 대한 정보가 손실되고 그러므로 모든 미확인 메시지가 <u>redelivered</u> 플래그 설정과 함께 재전송된다. consumer는 이 일이 일어날 것이라는 것을 알고 싶어할 것이다.

  그렇다면 <u>x-cancel-on-ha-failover</u> 설정을 true로 설정하여 사용할 수 있다. 페일오버 시 consumer의 소비가 취소되고 소비자 취소 알림이 전송되면 consumer의 소비는 취소된다. 다시 소비를 시작하기 위해 <u>basic.consume</u>을 재발행하는 것은 consumer의 책임이다.



- **비동기 미러(Unsynchronised Mirrors)**

  노드는 언제든지 클러스터에 참여할 수 있다. 큐의 구성에 따라 노드가 클러스터에 참여할 때 큐는 새 노드에 미러를 추가할 수 있다. 이 시점에서 새 미러는 비어 있을 것이다. 즉, 큐의 기존 내용을 포함하지 않는다. 이러한 미러는 큐로부터 게시된 새 메시지를 수신하므로 시간이 지남에 따라 미러된 큐의 꼬리(tail)를 정확하게 나타낼 것이다. 메시지가 미러링된 큐에서 유출됨에 따라 새로운 미러에서 메시지가 누락된 큐의 헤드 크기는 결국 미러의 내용이 마스터의 내용과 정확히 일치할 때까지 줄어든다. 이 시점에서 미러는 완전히 동기화된 것으로 간주될 수 있지만 기존에 존재하던 큐의 헤드를 유출하는 측면에서 클라이언트의 동작으로 인해 이러한 현상이 발생했다는 점에 유의해야 한다.

  새로 추가된 미러는 큐가 분명하게 동기화되지 않은 한 미러를 추가하기 전에 존재했던 큐 내용의 중복성이나 가용성의 추가 형식을 제공하지 않는다. 큐는 분명한 동기화가 발생하는 동안 응답하지 않게(무반응) 되므로 메시지가 유출되는 활성(active) 큐가 자연스럽게 동기화되고 비활성(inactive) 큐만 분명하게 동기화하도록 허용하는 것이 좋다.

  자동 큐 미러링을 사용하도록 설정할 때는 관련 큐의 디스크 데이터 세트에 대해 예상되는 값을 고려해야 한다. 크기가 큰 데이터 세트(예: 수십 기가 바이트 이상)가 있는 큐는 새로 추가된 미러로 복제해야 하므로 네트워크 대역폭과 디스크 I/O와 같은 클러스터 리소스(자원)에 상당한 부하를 줄 수 있다. 
...
- **실패시 비동기 미러 승격(Promotion of Unsynchronised Mirrors on Failure)**
...
- **기본 가상 호스트 및 사용자**
  ...
  서버가 처음 실행을 시작하고 해당 데이터베이스가 초기화되지 않았거나 삭제된 것을 감지했을 때 다음과 같은 자원(resources)으로 새 데이터베이스를 초기화한다.

  - 가상 호스트 이름 <u>/</u>
  - <u>guest</u>의 기본 비밀번호를 가진 <u>guest</u>라는 사용자, <u>/</u> 가상호스트에 대한 전체 액세스 권한이 부여된다.



- **"guest"라는 사용자는 localhost를 통해서만 연결할 수 있다.**

  기본적으로 guest 사용자는 원격 호스트에서 연결할 수 없다. loopback 인터페이스(즉, localhost)를 통해서만 연결할 수 있다. 이것은 AMQP 0-9-1 및 플러그인을 통해 활성화된 다른 모든 프로토콜에 모두 적용된다.
...
- **권한 작동 방식(How Permissions Work)**

  RabbitMQ 클라이언트는 서버에 대한 연결을 설정하고 인증할 때, 작동하려는 가상 호스트를 지정한다. 이 단계에서 서버는 사용자가 가상 호스트에 액세스하기 위한 권한이 있는지 여부를 확인하고 그렇지 않으면 연결 시도를 거부하여 첫 번째 수준의 액세스 제어가 진행된다.

  자원(즉, 교환 및 큐)은 특정 가상 호스트 내부의 명명된 엔티티로 동일한 이름은 각 가상 호스트의 다른 리소스(자원)를 의미한다. 특정 작동이 자원에 대해 수행될 경우 두 번째 레벨의 액세스 제어가 진행된다.

  RabbitMQ는 자원에 대한 구성, 쓰기 및 읽기 작업을 구분한다. 구성작업은 자원을 만들거나 파괴하거나, 해당 자원의 행동을 변경한다. 쓰기 작업은 자원에 메시지를 넣고 읽기 작업은 자원에서 메시지를 검색한다. 자원에 대한 작업을 수행하려면 사용자는 적절한 권한을 부여받아야 한다. 



- **사용자 태그 및 관리 UI 액세스(User Tags and Management UI Access)**

  사용자는 태그를 가질 수 있다. 현재 관리 UI 액세스만 사용자 태그에 의해 제어된다. 태그는 rabbitmqctl을 사용하여 관리되며 새로 생성된 사용자에게는 기본적으로 태그가 설정되어 있지 않다.



- **topic 승인(Topic Authorisation)**

  버전 3.7.0부터 RabbitMQ는 topic 교환에 대한 topic authorisation을 지원한다. topic 교환에 게시된 메시지의 라우팅 키는 게시 승인이 적용될 때 고려된다. Topic authorisation 은 STOMP와 MQTT 같은 프로토콜을 대상으로 한다.

  topic authorisation은 producer를 위한 기존의 checks 위에 추가 레이어이다. topic 유형 exchange에 메시지를 게시하는 것은 basic.publish와 라우팅 키 checks를 모두 거치게 된다. 

  topic authorisation은 topic consumer에게도 적용될 수 있다. 프로토콜이 따라 다르게 작동한다는 점에 유의해야 한다. Topic authorisation의 개념은 MQTT 및 STOMP와 같은 topic 지향 프로토콜에 대해서만 의미가 있다. 예를 들어, AMQP 0-9-1의 경우 consumer는 큐로부터 소비하므로 표준 자원 권한이 적용된다. 또한 AMQP 0-9-1에서 AMQP 0-9-1 topic 교환과 큐/교환 간의 바인딩 라우팅 키는 구성된 topic 권한과 비교하여 확인된다. 

  기본 승인 백엔드를 사용할 경우 topic 교환에 게시하거나 topic으로부터 소비하는 것은 topic 사용 권한이 정의되지 않은 경우 항상 승인된다. 이 승인 백엔드에서는 Topic authorisation이 선택 사항이므로 교환을 허용할 필요가 없다. 따라서 Topic authorisation을 사용하려면 하나 이상의 교환에 대한 topic 권한을 선택하고 정의해야한다.

  내부(기본) 권한 부여 백엔드는 권한 패턴의 변수 확장을 지원한다. 지원되는 변수는 username , vhost 및 client_id이다.



- **백엔드 결합(Combining Backends)**
...
RabbitMQ는 다양한 SASL 인증 매커니즘을 지원한다. 서버에는 <u>PLAIN</u>, <u>AMQPLAIN</u> 및 <u>RABBIT-CR-DEMO</u>의 세 가지 매커니즘이 내장되어 있으며, 플러그인에서 rabbit_auth_mechanism 동작을 구현하여 고유한 인증 매커니즘을 구현할 수도 있다.

- **내장 매커니즘(Built-in Mechanisms)**

  내장 매커니즘은 다음과 같다.

  - <u>PLAIN</u> : SASL PLAIN 인증. RabbitMQ 서버 및 클라이언트에서 기본적으로 활 성화되며 대부분의 다른 클라이언트의 기본값이다.
  - <u>AMQPLAIN</u> : PLAIN의 비표준 버전은 이전 버전과의 호환성을 유지한다. RabbitMQ 서버에서 기본적으로 활성화된다.
  - <u>EXTERNAL</u> : 인증은 x509 인증서 피어 확인 , 클라이언트 IP 주소 범위 등과 같은 대역 외 매커니즘을 사용하여 수행된다. 이러한 매커니즘은 일반적으로 Rabbit  MQ 플러그인에 의해 제공된다.
  - <u>RABBIT-CR-DEMO</u> :  challenge-response 인증을 나타내는 비표준 매커니즘. 이 매커니즘은 PLAIN과 동등한 보안 기능을 제공하며 RabbitMQ 서버에서 기본적으 로 활성화되지 않는다.



- **서버 매커니즘 구성(Mechanism Configuration in the Server)**

  <u>rabbit</u> 응용 프로그램의 구성 변수 auth_mechanisms는 클라이언트를 연결하는 데 설치된 메커니즘을 결정한다. 이 변수는 기본적으로 ['PLAIN', 'AMQPLAIN']과 같이 메커니즘 이름에 해당하는 원자 목록이어야 한다. 서버 측 목록은 특정 순서로 간주되지 않는다.



- **클라이언트 매커니즘 구성(Mechanism Configuration in the Client)**

  - Java의 매커니즘 구성(Mechanism Configuration in Java)

    Java 클라이언트는 <u>javax.security.sasl</u> 패키지를 기본적으로 사용하지 않는다. 이 는 Oracle 이외의 JDK에서 예측할 수 없으며 완전히 Android에서 누락되어 있기 때문이다. <u>SaslConfig</u> 인터페이스로 구성된 RabbitMQ 관련 SASL 구현이 있다. 클래스 <u>DefaultSaslConfig</u>는 SASL 구성을 일반적인 경우보다 편리하게 하기 위해 제공된다. <u>JDKSaslConfig</u> 클래스는 <u>javax.security.sasl</u>에의 브릿지로서 기능하기 위해서 제공되고 있다.

    <u>ConnectionFactory.getSaslConfig()</u> 및 <u>ConnectionFactory.setSaslConfig(SaslConfig)</u>는 인증 매커니즘과 상호작용하기 위한 주요 방법이다.

    

  - Erlang의 매커니즘 구성(Mechanism Configuration in Erlang)

    Erlang 클라이언트는 amqp_auth_mechanisms 모듈에 자체 SASL 메커니즘 구현을 제공한다. #amqp_params{} 레코드는 네트워크 연결의 우선순위에 따라 인증 기 능의 목록을 제공할 수 있다.

    

  - .NET의 매커니즘 구성(Mechanism Configuration in .NET)

    .Net 클라이언트는 <u>AuthMechanism</u> 및 <u>AuthMechanismFactory</u> 인터페이스를 기반으로 자체 SASL 메커니즘 구현을 제공한다. 

    <u>ConnectionFactory.AuthMechanisms</u> 속성은 인증 메커니즘 팩토리의 우선순위 목록이다.



- **권한 부여 문제 해결(Troubleshooting Authorisation)**
...
기본적으로 큐는 메시지가 RabbitMQ에 게시될 때 채워지는 메시지의 메모리 내 캐시를 유지한다. 이 캐시의 개념은 consumer에게 가능한 한 빨리 메시지를 전달할 수 있다는 것이다. 영구 메시지는 브로커에 들어가 동시에 RAM에 보관될 때 디스크에 기록될 수 있다는 점에 유의해야 한다.

브로커가 메모리를 확보해야 한다고 생각할 때마다 이 캐시의 메시지는 디스크로 페이징된다. 디스크에 메시지를 일괄적으로 페이징하는 데 시간이 걸리고 큐 프로세스가 차단되므로 페이징하는 동안 새 메시지를 수신할 수 없다. 비록 최근 버전의 RabbitMQ가 페이징 알고리즘을 개선시켰지만, 이 상황은 여전히 페이징 알고리즘을 페이징해야 할 수 있는 큐에 수백만 개의 메시지가 있는 사용 사례에 적합하지 않다.

그리고 지연 큐들은 메시지를 가능한 빨리 디스크로 옮기려고 시도한다. 이는 대부분의 경우 정상 작동 시 RAM에 보관되는 메시지 수가 상당히 적다는 것을 의미한다. 이는 디스크 I/O 증가시키는 비용에서 발생한다.



- **지연 큐 만들기(Making a Queue Lazy)**

  큐는 <u>default</u> 모드 도는 <u>lazy</u> 모드로 실행되도록 만들 수 있다.

  - <u>queue.declare</u> 인수를 통해 모드 설정
  - 큐 정책 사용

  policy(정책) 및 queue 인수가 모두 큐 모드를 지정하면 queue 인수는 policy 값보다 우선순위를 갖는다.



- **선언시 인수 사용(Using Arguments at the Time of Declaration)**

  큐 모드는 <u>x-queue-mode</u> 큐 선언 인수에 원하는 모드를 지정하는 문자열을 제공하여 설정할 수 있다.
...
- **Policy 사용(Using a policy)**

  Policy를 사용하여 큐 모드를 지정하려면 키 <u>queue-mode</u>를 정책 정의에 추가해야 한다.

  다음 표는 lazy-queue라 불리는 큐를 lazy 모드로 작동시킨다.

  | rabbitmqctl                                                  | rabbitmqctl(Windows)                                         |
  | ------------------------------------------------------------ | ------------------------------------------------------------ |
  | rabbitmqctl set_policy Lazy "^lazy-queue$" '{"queue-mode":"lazy"}' --apply-to queues | rabbitmqctl set_policy Lazy "^lazy-queue$" "{""queue-mode"":""lazy""}" --apply-to queues |

  

- **런타임에 큐 모드 변경(Changing Queue Mode at Runtime)**

  큐 모드가 정책을 통해 구성된 경우 큐를 삭제하고 다시 선언할 필요 없이 런타임에서 경할 수 있다. “lazy-queue”라는 이름의 큐를 기본(non-lazy) 모드로 사용하려면 일치하는 정책을 업데이트하여 다른 <u>queue-mode</u>를 지정하면 된다.

  | rabbitmqctl                                                  | rabbitmqctl(Windows)                                         |
  | ------------------------------------------------------------ | ------------------------------------------------------------ |
  | rabbitmqctl set_policy Lazy "^lazy-queue$" '{"queue-mode":"default"}' --apply-to queues | rabbitmqctl set_policy Lazy "^lazy-queue$" "{""queue-mode"":""default""}" --apply-to queues |

  

- **지연 큐의 성능 고려 사항(Performance Considerations for Lazy Queues)**

  - 디스크 사용률

    지연 큐는 producer가 메시지를 일시적으로 게시한 경우에도 가능한 한 빨리 메시 지를 디스크로 메시지를 이동시킬 것이다. 그러면 일반적으로 I/O 사용률이 높아진 다.

    일반적인(regular) 큐는 메시지를 더 오랫동안 메모리에 보관한다. 이로 인해 더 많은 데이터를 디스크에 한 번에 기록해야 하기 때문에 디스크 I/O가 지연되는 결과를 초래한다.



- **RAM 사용률(RAM Utilization)**

  모든 사용 사례에서 정확한 숫자를 제공하는 것은 불가능하지만, 이는 일반 및 지연 큐 간의 RAM 사용량의 차이를 보여주는 단순한 테스트이다.

  | Number of messages | Message body size | Message type | Producers | Consumers |
  | ------------------ | ----------------- | ------------ | --------- | --------- |
  | 1,000,000          | 1,000bytes        | persistent   | 1         | 0         |

  위의 메시지를 처리한 후 기본 및 지연 큐의 RAM 사용률
  ...
  두 큐 모두 1,000,000개의 메시지를 유지하고 1.2GB의 디스크 공간을 사용한 것을 확인할 수 있다.



- **런타임에 큐 모드 전환(Switching Queue Mode at Runtime)**

  <u>default</u> 큐를 <u>lazy</u> 큐로 변환할 때 큐에서 메시지를 디스크로 이동해야 할 때와 동일한 성능 영향을 받게 된다.

  일반 모드에서 지연 모드로 변환하는 동안 큐는 RAM에 저장된 모든 메시지를 먼저 디스크에 표시한다. 이 작업이 진행되는 동안 게시하는 채널로부터 더 이상 메시지를 수신하지 않는다. 초기 페이지아웃이 완료되면 큐에서 게시, 확인 및 다른 명령을 수신하기 시작한다.

  큐가 지연모드에서 기본 모드로 변경되면 서버가 다시 시작된 후 큐가 복구될 때와 동일한 프로세스가 수행된다.



- **혼합된 메시지 크기의 지연 큐(Lazy Queues with Mixed Message Sizes)**

  처음 10,000개의 메시지의 모든 메시지가 queue_index_embed_msgs_below 아래 값이고 나머지는 이 값보다 큰 경우 노드 시작 시 처음 10,000개만 메모리에 로드된다.



- **인터리브 메시지의 지연 큐(Lazy Queues with Interleaved Message)**

  주어진 인터리브된(interleaved) 메시지 크기 :
  ...
  queue_index_embed_msgs_below 아래 값의 처음 20개 메시지만 노드 시작 시 메모리로 로드된다. 이 시나리오에서 메시지는 21KB 의 시스템 메모리를 사용하고 큐 프로세스는 다른 32KB의 시스템 메모리를 사용한다. 큐 프로세스가 시작을 완료하는데 필요한 전체 시스템 메모리는 53KB이다.



# Firehose(Message Tracing)

RabbitMQ에는 “Firehose” 기능이 있다. 관리자는 이를 통해 게시 및 delivery 통지되어야 하는 교환을 (노드별로, 호스트별로) 설정할 수 있다.

- **Firehose 활성화**
  1. 어떤 노드와 어떤 가상 호스트를 활성화할지 결정한다. 
  2. 선택한 가상 호스트에서 큐를 생성하고 교환 <u>amq.rabbitmq.trace topic exchange</u>에 바인드한 후 consuming을 시작한다.
  3. rabbitmqctl trace_on을 실행한다.



- **Firehose 비활성화**

  1. rabbitmqctl trace_off를 실행한다.
  2. Firehose가 사용하는 큐를 정리한다.

  Firehose 상태는 지속되지 않으며 서버 시작 시 기본 값은 off가 된다.
...
- **Firehose 알림 형식**
  ...
  - original 메시지에 대한 metadata를 포함하는 헤더 : 
    ...
    | redelivered   | signedint | 메시지에 재전달 플래그가 설정되었는지 |
